...
public class InstallService {
    ...
    public void dbInstall() {
        ...
        try(Connection conn = DriverManager.getConnection("jdbc:sqlite:"+path+"?"+decryptor(getDbValue()))) {
            ...
            stmt.execute("CREATE TABLE USER (ID TEXT, PASS TEXT NOT NULL, NICKNAME TEXT NOT NULL, EMAIL TEXT, AUTH TEXT NOT NULL DEFAULT N, NEED_VERIFY TEXT DEFAULT 'Y', GROUP_NO INTEGER NOT NULL DEFAULT 1" +
                            ", PRIMARY KEY(ID)" +
                            ", FOREIGN KEY(GROUP_NO) REFERENCES GROUP_SETTING(NO))");
